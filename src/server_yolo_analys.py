...
class server_yolo_analys():
	...
	async def __create_log_file(self):
		...
		path_to_log = f"{getcwd()}{slash}logs{slash}server_yolo_analys_log{slash}runtime_server_get_vidio_{self.__get_time()}.log"
	...
	async def __frame_sending(self, frame, user_id):
		self.redis.set(f"{user_id}_{user_id}", m.packb(frame.render()))
		self.producer.produce("processed_images", value=f"{user_id}_{user_id}".encode('utf-8'))
		self.producer.poll(0)

	@logger.catch
	async def __analyzing_and_sending_frame(self, msg):
		frame = self.model(m.unpackb(self.redis.get(msg)))
		task = create_task(self.__frame_sending(frame, msg))
		await task
